...
mock.onGet('/api/e-commerce-app/categories').reply(() => {
    return [200, eCommerceDB.categories];
});
...
mock.onGet('/api/e-commerce-app/category').reply((request) => {
    const {categoryId} = request.params;
    const response = _.find(eCommerceDB.categories, {id: categoryId});
    return [200, response];
});

mock.onPost('/api/e-commerce-app/category/save').reply((request) => {
    const data = JSON.parse(request.data);
    let category = null;

    eCommerceDB.products = eCommerceDB.categories.map(_category => {
